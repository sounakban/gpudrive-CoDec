...
And the following flies:
2. **cuda_environment.yml**: Can be used to first set up an environment for using GPUDrive on GPU (Ubuntu) using:

    ``conda env create -f environment.yml``
     Then follow instructions in **packageInstall.txt** to set up GPUDrive. If using CPU, follow instructions on main README.

1. **packageInstall.txt**: Contains instructions for setting up gpuDrive using GPU on an Ubuntu system. Usually, newer versions of Ubuntu use recent CUDA versions, which GPUDrive does not support. So, these instructions allow GPUDrive to use an older (supported) version of CUDA set up in a python environment.
