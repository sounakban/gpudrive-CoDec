...
def get_trajectory_overlap(trajectory1, trajectory2):
    """
    ...
    if intersection.is_empty:
      ...
      return [intersect_point for intersect_line in intersection.geoms for intersect_point in intersect_line.coords]
        ...
        raise Warning("Intersection type ", intersection.geom_type, " did not match any predefined types")
        return []
    # Example usage:
...
RATIO_COMPLEXITY = 1
INTERSECTION_COMPLEXITY = 2
OVERLAP_COMPLEXITY = 3
def get_traj_complexity(veh_obj, all_veh_objs, complexity_type = 2):
    """
    ...
    Args:
        ...
        complexity_type: The type of trajectory complexity to use.

    Returns:
        A dictionary with all complexity-related measures (varies by complexity type).
    """
    if complexity_type == RATIO_COMPLEXITY:
    ...
    elif complexity_type == INTERSECTION_COMPLEXITY:
        # |Complexity based path intersections
        # |Complexity based path overlaps
        ...
        for veh_obj2 in moving_vehs:
            ...
            if len(veh2_pos) < 2: 
                intersection_list.append(0)
                continue
            intersection_list.append(1 if get_trajectory_overlap(veh1_pos, veh2_pos) else 0)
        traj_complexity = sum(intersection_list)
    ...
    elif complexity_type == OVERLAP_COMPLEXITY:
        # |Complexity based path overlaps
        moving_vehs = get_moving_vehicles(all_veh_objs)
        # veh1_pos = [[posdict['x'],posdict['y'],posdict['z']] for posdict in veh_obj['position'] if posdict['x']!=-10000]
        veh1_pos = [[posdict['x'],posdict['y']] for posdict in veh_obj['position'] if posdict['x']!=-10000]
        ...
        for veh_obj2 in moving_vehs:
            ...
            if len(veh2_pos) < 2: 
                intersection_list.append(0)
                continue
            intersection_list.append(len(get_trajectory_overlap(veh1_pos, veh2_pos)))
        ...
        raise ValueError("Complexity type not recognized")
...
def fileterScenes_1(dirpath):
    ...
    for scFile in scenario_files:
        ...
        traj_complexity[scFile] = get_traj_complexity(moving_veh[0], data['objects'], complexity_type = OVERLAP_COMPLEXITY)
...
def fileterScenes_3(env):
    ...
    for scFile, veh_num in zip(data_files, control_mask.nonzero()):
        ...
        traj_complexity[scFile] = get_traj_complexity(contVeh, data['objects'], complexity_type = RATIO_COMPLEXITY)
    return traj_complexity
